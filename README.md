...
See the [parity matrix](docs/parity-matrix.md) for exact Codex completed and partial contracts. The larger approved Flauz architecture and implementation sequence lives in [the architecture constitution](docs/FLAUZ-SOURCE-OF-TRUTH.md) and [master roadmap](docs/IMPLEMENTATION-ROADMAP.md). The largest remaining gaps are full Linux Computer Use, scheduled
