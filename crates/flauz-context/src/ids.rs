...
//!
//! This crate owns the typed identifiers for the context entities
//! ([`ContextSnapshotId`] `ctxsnap_` and [`MemoryItemId`] `mem_`). Foreign
//! crates' entities are referenced through validated local newtypes —
//! [`TaskRef`], [`SessionRef`], [`EventRef`], [`ArtifactRef`],
//! [`ObservationRef`], [`EnvironmentRef`] and [`ModelRef`] — which validate
//! the frozen `<kind>_<ULID>` grammar without ever defining the foreign
//! entity types (kernel §1 ownership rules).
...
/// vectors (§2) pin one reason per invalid vector; the reasons are identical
/// across the F2 crates.
...
/// example `task_` or `model_` IDs referenced from context state).
...
canonical_id!(
    /// Canonical ContextSnapshot identifier (`ctxsnap_<ULID>`), owned by
    /// this crate (kernel §2 registry).
    ContextSnapshotId,
    "ctxsnap"
);
canonical_id!(
    /// Canonical MemoryItem identifier (`mem_<ULID>`), owned by this crate
    /// (kernel §2 registry).
    MemoryItemId,
    "mem"
);
canonical_id!(
    /// A validated reference to a foreign Task (`task_<ULID>`, owned by
    /// flauz-world). A Task is identified solely by its canonical ID
    /// (kernel §6); context state references tasks without defining the
    /// Task entity.
    TaskRef,
    "task"
);
canonical_id!(
    /// A validated reference to a foreign Session (`sess_<ULID>`, owned by
    /// flauz-world). A Session references at most one Task and is never
    /// itself the context (kernel §6).
    SessionRef,
    "sess"
);
canonical_id!(
    /// A validated reference to a foreign Event (`ev_<ULID>`, owned by
    /// flauz-world), for example the durable session events a context item
    /// is attributable to.
    EventRef,
    "ev"
);
canonical_id!(
    /// A validated reference to a foreign Artifact (`art_<ULID>`, owned by
    /// flauz-world).
    ArtifactRef,
    "art"
);
canonical_id!(
    /// A validated reference to a foreign Observation (`obs_<ULID>`, owned
    /// by flauz-world), the durable record behind a resource-observation
    /// context source.
    ObservationRef,
    "obs"
);
canonical_id!(
    /// A validated reference to a foreign Environment (`env_<ULID>`, owned
    /// by flauz-exec). An Environment is where execution happens; it is
    /// never the Model and never the Agent.
    EnvironmentRef,
    "env"
);
canonical_id!(
    /// A validated reference to a foreign Model (`model_<ULID>`, owned by
    /// flauz-exec). A Model is intelligence; model identity is execution
    /// state, never task identity (kernel §6).
    ModelRef,
    "model"
);
...
mod tests {
    ...
    fn typed_parse_rejects_valid_ulid_of_wrong_kind() {
        let error = TaskRef::parse("ws_01J8ZQ5V8K3T2B7N6X4R9DQPA0");
        assert_eq!(
            error.err(),
    ...
    fn typed_parse_accepts_its_own_kind() {
        let id = TaskRef::parse("task_01J8ZQ5V8K3T2B7N6X4R9DQPB1");
        assert_eq!(
