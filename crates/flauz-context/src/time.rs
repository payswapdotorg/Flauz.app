...
impl Timestamp {
    ...
    pub fn parse(value: &str) -> Result<Self, ContextError> {
        let parsed = DateTime::parse_from_rfc3339(value).map_err(|error| {
            ContextError::invalid(format!("invalid RFC 3339 timestamp {value:?}: {error}"))
        })?;
    ...
    pub fn from_unix_seconds(seconds: i64) -> Result<Self, ContextError> {
        let value = DateTime::from_timestamp(seconds, 0).ok_or_else(|| {
            ContextError::invalid(format!("unix timestamp {seconds} is out of range"))
        })?;
        Ok(Self(value))
    }
