...
impl Timestamp {
    ...
    pub fn parse(value: &str) -> Result<Self, ExecError> {
        let parsed = DateTime::parse_from_rfc3339(value).map_err(|error| {
            ExecError::invalid(format!("invalid RFC 3339 timestamp {value:?}: {error}"))
        })?;
    ...
    pub fn from_unix_seconds(seconds: i64) -> Result<Self, ExecError> {
        let value = DateTime::from_timestamp(seconds, 0).ok_or_else(|| {
            ExecError::invalid(format!("unix timestamp {seconds} is out of range"))
        })?;
        Ok(Self(value))
    }
...
mod tests {
    ...
    fn unix_seconds_round_trip() {
        let ts = ok(Timestamp::from_unix_seconds(1_789_998_300));
        assert_eq!(ts.to_unix_seconds(), 1_789_998_300);
        assert_eq!(ts.to_rfc3339(), "2026-09-21T13:45:00Z");
