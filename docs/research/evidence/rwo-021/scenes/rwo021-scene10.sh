...
Xvfb $DISP -screen 0 1600x1000x24 -nolisten tcp -noreset >"$OUT/xvfb10.log" 2>&1 &
sleep 1.5
DISPLAY=$DISP LD_LIBRARY_PATH="$LD_LIBRARY_PATH" picom --backend xrender --config /dev/null >"$OUT/picom10.log" 2>&1 &
sleep 3
HOME_D=$(mktemp -d /tmp/rwo021-home10.XXXXXX)
mkdir -p "$HOME_D/xdg"
...
for i in $(seq 1 30); do
  ...
  cap "$OUT/_probe10.png" 2>/dev/null
  SZ=$(stat -c%s "$OUT/_probe10.png" 2>/dev/null || echo 0)
...
cap "$OUT/_j00.png"
S_MAIN=$(ssim_of "$OUT/_j00.png" "$MAIN_REF"); S_SET=$(ssim_of "$OUT/_i00.png" "$SET_REF")
...
cap "$OUT/j01-composer-feedback-typed.png"
key ctrl+Return; sleep 2.5
cap "$OUT/j02-feedback-dialog-ctrl-enter.png"
key Escape; sleep 1.5
echo "--- [i03] overlay filtered query (matcher GUI evidence)"
key ctrl+slash; sleep 2.5
type_ "search"; sleep 1.8
cap "$OUT/j03-overlay-filtered-search.png"
key Escape; sleep 1
