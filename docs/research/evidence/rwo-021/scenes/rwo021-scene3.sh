#!/bin/bash
# RWO-021 scene 3 — corrected coordinates: Back to app, composer /feedback,
# sidebar PR/Plugins pages, settings search "import", shortcuts-page filter.
set -u
...
Xvfb $DISP -screen 0 1600x1000x24 -nolisten tcp -noreset >"$OUT/xvfb3.log" 2>&1 &
sleep 1.5
DISPLAY=$DISP LD_LIBRARY_PATH="$LD_LIBRARY_PATH" picom --backend xrender --config /dev/null >"$OUT/picom3.log" 2>&1 &
sleep 3

HOME_D=$(mktemp -d /tmp/rwo021-home3.XXXXXX)
mkdir -p "$HOME_D/xdg"
...
for i in $(seq 1 30); do
  ...
  cap "$OUT/_probe3.png" 2>/dev/null
  SZ=$(stat -c%s "$OUT/_probe3.png" 2>/dev/null || echo 0)
...
echo "--- [c01] restored state"
cap "$OUT/c01-restored.png"
echo "--- [c02] Back to app"
click 295 315; sleep 2.5
cap "$OUT/c02-main-surface.png"
echo "--- [c03] composer typed /feedback (FBK-C5 slash leg)"
click 700 770; sleep 1
type_ "/feedback"; sleep 1.5
cap "$OUT/c03-composer-feedback-typed.png"
key Return; sleep 2.5
cap "$OUT/c04-feedback-dialog-slash.png"
key Escape; sleep 1.5
echo "--- [c05] sidebar Pull requests (NTR-C5)"
click 300 222; sleep 3
cap "$OUT/c05-sidebar-pullrequests.png"
echo "--- [c06] sidebar Plugins (NTR-C5)"
click 300 258; sleep 3
cap "$OUT/c06-sidebar-plugins.png"
echo "--- [c07] settings + search import (SET-C3)"
key ctrl+comma; sleep 3
cap "$OUT/c07-settings.png"
click 400 430; sleep 0.8
type_ "import"; sleep 1.8
cap "$OUT/c08-settings-search-import.png"
key ctrl+a; key Delete; sleep 0.8
echo "--- [c09] shortcuts page + filter (KSR-C4)"
key ctrl+k; sleep 1.5; type_ "keyboard"; sleep 1.2; key Return; sleep 2.5
cap "$OUT/c09-settings-kbshortcuts.png"
click 800 250; sleep 0.8
type_ "search"; sleep 1.8
cap "$OUT/c10-kbshortcuts-filtered.png"

kill -0 $APP_PID 2>/dev/null && echo "APP ALIVE AT SCENE-3 END" || echo "APP EXITED"
kill $APP_PID 2>/dev/null; sleep 1; pkill -f "Xvfb $DISP" 2>/dev/null
for f in "$OUT"/c*.png; do echo "$(basename "$f"): $(md5 "$f")"; done > "$OUT/scene3-md5.txt"
cat "$OUT/scene3-md5.txt"
echo "SCENE3_COMPLETE"
