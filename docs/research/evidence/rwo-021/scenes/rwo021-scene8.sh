#!/bin/bash
# RWO-021 scene 8 — composer submit via Shift+Enter (PressEnter{secondary:true}); KSR-C4 filter retry
set -u
...
Xvfb $DISP -screen 0 1600x1000x24 -nolisten tcp -noreset >"$OUT/xvfb8.log" 2>&1 &
sleep 1.5
DISPLAY=$DISP LD_LIBRARY_PATH="$LD_LIBRARY_PATH" picom --backend xrender --config /dev/null >"$OUT/picom8.log" 2>&1 &
sleep 3
HOME_D=$(mktemp -d /tmp/rwo021-home8.XXXXXX)
mkdir -p "$HOME_D/xdg"
...
for i in $(seq 1 30); do
  ...
  cap "$OUT/_probe8.png" 2>/dev/null
  SZ=$(stat -c%s "$OUT/_probe8.png" 2>/dev/null || echo 0)
...
cap "$OUT/_h00.png"
S_MAIN=$(ssim_of "$OUT/_h00.png" "$MAIN_REF"); S_SET=$(ssim_of "$OUT/_h00.png" "$SET_REF")
...
cp "$OUT/_h00.png" "$OUT/h00-main.png"

echo "--- [h01/h02] composer /feedback + Shift+Enter"
click 748 841; sleep 1
type_ "/feedback"; sleep 1.2
cap "$OUT/h01-composer-feedback-typed.png"
key shift+Return; sleep 2.5
cap "$OUT/h02-feedback-dialog-shift-enter.png"
if [ "$(md5 "$OUT/h01-composer-feedback-typed.png")" = "$(md5 "$OUT/h02-feedback-dialog-shift-enter.png")" ]; then
  echo "shift+Enter no change — trying send button"
  key Escape; sleep 1
  click 1030 850; sleep 2.5
  cap "$OUT/h03-feedback-dialog-send-button.png"
fi
key Escape; sleep 1.5

echo "--- [h04] KSR-C4 filter retry: shortcuts page, click input (700,404), type 'search'"
key ctrl+comma; sleep 3
click 300 203; sleep 0.6
key ctrl+a; key Delete; sleep 0.8
click 300 382; sleep 2.5
cap "$OUT/h04-settings-kbshortcuts.png"
click 700 404; sleep 0.8
type_ "search"; sleep 1.8
cap "$OUT/h05-kbshortcuts-filtered.png"

kill -0 $APP_PID 2>/dev/null && echo "APP ALIVE AT SCENE-8 END" || echo "APP EXITED"
kill $APP_PID 2>/dev/null; sleep 1; pkill -f "Xvfb $DISP" 2>/dev/null
for f in "$OUT"/h*.png; do echo "$(basename "$f"): $(md5 "$f")"; done
echo "SCENE8_COMPLETE"
