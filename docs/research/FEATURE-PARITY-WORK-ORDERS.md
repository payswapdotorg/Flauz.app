...
- **Tests:** 7 codex-core state tests (background completion marking incl. failed turns + selected/unknown exclusions; approval marking; visit-clears; manual toggle round-trip; archive drops; clear-all honest reporting; activity-view honest guidance) — verified locally with per-test output; 6 codex-app binding tests (exact-one-owner accelerators, registry membership + metadata, reducer resolution, jump semantics) — CI double matrix.
...
- Dispatched 2026-09-18 (wave S): persistent browsing-history store + revisit matching + Settings management + reload/copy-URL keybindings (WO-R-REF R2 scope). Original session destroyed in the 22:31–22:42 rate-limit purge (server-verified http-500); re-dispatch fighting the GLM-5.3 evening-capacity gate under `recover_capacity.py` (serialized, lesson 89b) as of 2026-09-19 00:2x. Watcher armed; harvest on delivery.

### WO-P2-010 (IN FLIGHT)

- Dispatched 2026-09-18 (wave S): palette rows for the R3 command delta (absent commands enumeration). Original session destroyed in the same purge; re-dispatch under `recover_capacity.py` (serialized) as of 2026-09-19 00:1x. Watcher armed; harvest on delivery.
