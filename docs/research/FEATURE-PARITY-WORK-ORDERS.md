...
> **Status: ACTIVE — implementation wave 3 in flight: WO-P2-004
> (palette settings-page indexing) implemented on
> `parity/wo-p2-004-palette-settings` (PR #16).** Wave 2 landed: WO-P1-003
> CLOSED on main (merge `d06ae3b` via PR #15, 2026-09-17). Operator
...
- **Title:** Command palette does not index all settings pages — official reference now runtime-observed on Linux: the official palette's dynamic Settings group lists 10 pages at the login surface (General, Import, Appearance, Voice, Pets, Git, Connections, Environments, Worktrees, Configuration) `[runtime-observed: linux-preview 26.908.70816; evidence docs/research/evidence/codex-linux/03-04]` — status: **IMPLEMENTED (PR #16)** [implemented on `parity/wo-p2-004-palette-settings` (5d4b083): six new palette commands (Profile, Import, Browser, Configuration, Hooks, Git) with title/description/icon mirroring the settings-nav rows, Settings palette group, no guards (nav-reachable surfaces are unauthenticated and repository-independent, matching the official login-surface reference); `PaletteCommand::ALL` 45 → 51; `Personalization` palette title aligned with its settings-nav label ("Personality" → "Personalization", description keeps "personality" findable); `SettingsSection::DEFAULT_NAV_SECTIONS` registry (15 default-nav sections; contextual CodeReview/Worktrees/ArchivedChats stay out per known limitations) + `settings_section` dispatch mapping (#[cfg(test)], house pattern); tests: registry coverage (every default-nav section has a palette entry; OpenPlugins is the documented Plugins alias) + the filtering contract (querying by each section's nav name resolves to an entry that opens that page); local gates: fmt, dependency policy (561), clippy --workspace --all-targets -D warnings, 603 tests (406 workspace + 197 codex-app harness); runtime GUI evidence docs/research/evidence/wo-p2-004/ (15 VLM-read captures: palette Settings group lists all 11 entries incl. the six new ones — folded + scrolled views; query "import" resolves — the ev/18 remediation — and navigates to the Import page; Profile/Browser/Configuration/Hooks/Git queries each resolve and navigate to their pages, unauthenticated, mirroring the official login-surface reference) — scene d7 + d7b passed]
- **Platform:** all (UI-shell concern; Linux-verifiable in lab)
