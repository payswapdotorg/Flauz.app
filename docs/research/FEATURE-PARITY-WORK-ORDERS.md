...
### WO-P2-017 (CLOSED — r2; r1 rejected at Lead verification)

- **ID:** WO-P2-017 + WO-P2-017-FIX (f1-sweep renumbered follow-up; a11y-inventory §(a)/(d) — palette/overlay close strands focus)
- **Title:** Palette and keyboard-shortcuts-overlay close restores focus to the previously focused surface (r2 eliminates the r1 close-path panic)
- **Scope delivered (r2, merged):** the full r1 a11y contract re-ported onto current main plus the borrow-safety fix — `focus_before_command_palette`/`focus_before_keyboard_shortcuts` capture on open (re-opens keep the original capture); on close, `apply_overlay_close_focus_restore` lands focus on the previously focused surface, else the composer house default when rendered (Tasks route), else `window.blur()` (no element focus — window-level chords keep working). The r1 panic eliminated by parameter-passing: `CommandPaletteCloseContext { files_mode }` captured via `capture(self.mode)` on the palette side BEFORE `self.workspace.update(...)` in both `close_and_clear` and `open_file_result`; `WorkspaceView::close_command_palette` takes the context argument and reads no palette entity; the workspace-side paths (palette backdrop, `toggle_keyboard_shortcuts` chord) read the idle palette through `command_palette_close_context(&self, cx: &App)` under a workspace lease only; the backdrop deliberately NOT restructured to a `palette.update(...)` delegation (would double-lease the workspace). Overlay close funnels through `close_workspace_modal` → `keyboard_shortcuts_close_restore` request-once consumed in `render` (the MCP-elicitation in-render precedent); the two-Escape overlay contract preserved (`keyboard_shortcuts_escape_closes_overlay`).
- **Acceptance criteria:** (1) implementing commits merged — PR #41 → `8dcfcb97b34bc9d4fd73bee534628d5e9c789abe` (worker delivery `a079caf`, single commit on base `c2de31e` exactly — re-verified at clone time per the work order's own rule, the addendum's `e4a9894` acknowledged stale; ui.rs-only +298/−25; + Lead rustfmt follow-up `1886ce2` — four layout-only sites per the CI Format step, the 019 house pattern); (2) tests green — CI double matrix green at `1886ce2` (Format, Clippy, full workspace Test, Release build, Linux smoke; worker sandbox had no Rust toolchain — compile/test/fmt/clippy rode CI, the UX-002/019 pattern); four focused tests: the two r1 focus-contract tests ported verbatim + `command_palette_close_context_captures_the_files_mode` (Files/Chats/Unified mapping) + `workspace_close_command_palette_requires_a_captured_close_context` (compile-time fn-pointer signature pin — structurally excludes the r1 shape; a runtime test cannot catch r1, whose unit tests passed while the binary crashed); (3) D17 GUI evidence at `1886ce2` (guarded-built binary `codexrs-wo017fix-1886ce2`): Ctrl+K → Escape closes the palette with NO PANIC (the r1 reproduction killed the app at exactly this step), typed-probe "x" lands in the composer (VLM-adjudicated), clear returns the exact closed-state md5; Ctrl+/ → Escape overlay round with probe "y" identical pattern; (4) diff confined to the prescribed file; (5) §5.10 Keyboard row cells + this row + changelogs (this closure).
- **Known limitations:** the backdrop close is now guarded (`if let Some(context)` — a deliberate defensive no-op when the palette is already None, unreachable while the backdrop renders); `open_thread_find` retains its pre-existing direct `command_palette = None` (no entity read, no panic shape, out of scope — flagged for a future tidy only); screen-reader labels/AT, reduced-motion, contrast remain the bounded F1 close-out slice (unchanged).
- **Provenance:** r1 (`feat/wo-p2-017-palette-close-focus` @ `5d3ab2e`) was REJECTED at Lead verification for the deterministic close-path panic (gpui entity-map double-lease at entity_map.rs:138 — "cannot read CommandPaletteView while it is already being updated"; r1 unit tests green, binary crashed on Ctrl+K, Escape — D17 reproduction). The verified-design addendum (root-cause analysis confirmed against the real gpui 0.2.2 crate; the parameter-passing fix shape; the fn-pointer pin) was baked into the r2 prompt. r2 fought the platform capacity regime across ~13 recycle generations (01:49–09:08 UTC; queue_watch unstick/assault + stall_recovery r2 dead-turn recycles; an earlier generation's server-confirmed COMPLETE at 02:23 was forensically closed as a report-shaped failure — no branch push, workspace recycled: the prompt-harvest-promptly lesson, since institutionalized in an autonomous harvest watcher). Winning session `2d94b1c4` (dispatched 08:44 through the assault chain) streamed 179K chars/165 blocks, report at 09:08, contract deviations NONE. Harvest via the workspaces files API (`ws-a67231d7`). Lead verification: full-diff source review against the addendum (every element confirmed); CI (no swallowed-Actions-event workaround needed this time); guarded build disk-tight at 304 MB free (019 mitigation playbook); D17 with both VLM probe adjudications.
- **Status:** CLOSED — PR #41 (`8dcfcb97b34bc9d4fd73bee534628d5e9c789abe`); CI green both matrices; evidence `docs/research/evidence/wo-p2-017/`.
...
- **ID:** WO-P2-020 (f1-sweep renumbered follow-up; F-A4 — the last evidenced silent no-op; f1-sweep D-3 decision: separate WO over the WO-P2-012 scope extension)
- **Title:** F-A4 — honest feedback for Ctrl+P without a workspace
- **Scope delivered:** the Files palette early-return (no workspace open) now emits the bounded honest status "Select a workspace before searching files." through the house status path (`files_palette_command_status` → `Action::SetStatus`, the WO-P2-008/012 copy family) instead of silence; `state_local_workspace_cwd` helper (status `None` when a workspace IS open — palette opens normally, Ctrl+K regression guard intact).
- **Acceptance criteria:** (1) implementing commits merged — PR #38 → `30572a43afcf3af6fa29d2b9da097467d74e7401` (worker r2 `a69e63b` on base `8172f6e`; r1 `8155f7c` + prior attempts self-archived under `archive/wo-p2-020-prior-attempt`; Lead integration merge with WO-P2-018 — clean auto-merge, disjoint ui.rs regions, focused battery 5P/0F on the integrated tree, the 005/006 sequential-merge house pattern); (2) tests green — 1 focused test (no-workspace status verbatim + workspace-open None + round-trip) + suite 226P/0F locally; CI double matrix green on the integrated head; (3) D20 GUI evidence (Ctrl+P fresh no-workspace state: verbatim status, persists at settle, no palette; Ctrl+K opens the Unified palette); (4) diff confined to the prescribed file; (5) §5.10 F-A4 gap + resolution cells + this row + changelogs (this closure).
- **Known limitations:** the status is visual-only (no AT announcement — platform-bound, f1-sweep close packet); other palette modes' workspace guards were never evidenced silent (unchanged).
- **Provenance:** f1-sweep renumbered follow-up; worker r2 `a69e63b` delivered pre-reset (r1 `8155f7c` superseded); post-reset Lead verification wave: guarded build distinct-hash `9a0b1513`, focused 1P/0F, suite 226P/0F, fmt clean, D20 PASS at display :110 (fresh empty state — no donor by design); integration merge verified locally before the PR (focused battery 5P/0F on the integrated tree).
- **Status:** CLOSED — PR #38 (`30572a43afcf3af6fa29d2b9da097467d74e7401`); CI green both matrices; evidence `docs/research/evidence/wo-p2-020/`.

### WO-UX-002 (CLOSED)

- **ID:** WO-UX-002 (dispatched from the RG-A11Y baseline battery findings — the keyboard-only journey contract's own clause; not in the original f1-sweep blocker set because the gap was unknown until the battery ran)
- **Title:** `begin_new_chat` focuses the composer (keyboard-only chat creation, RG-A11Y finding N1)
- **Scope delivered:** the `begin_new_chat_with_prompt` focus pattern applied to `begin_new_chat` — `composer.update(cx, |input, cx| { input.focus(window, cx); })` immediately after the `Action::BeginNewChat` dispatch (the dispatch path reads route/selection but never focuses, so the call is the last word); every entry point that routes to `begin_new_chat` inherits it (the Ctrl+N chord, the palette "New chat" row, the sidebar new-chat rows); `begin_new_chat_with_prompt` keeps its own focus (idempotent double-focus, the regression guard).
- **Acceptance criteria:** (1) single commit `18a1815` on base `c8f6831`, `crates/codex-app/src/ui.rs` only, +102 additive; (2) the focus call present, composer-targeted, positioned after the dispatch (source-verified + the ordering rationale read from the dispatch path); (3) tests green — 2 focused source-pin tests (WO-P2-011 house style) + reducer-semantics asserts, CI green on the full rust-ci matrix at the PR head; (4) contract deviations: none.
- **The finding (evidence):** the RG-A11Y baseline battery (WO-UX-001 scenes at main `8a68c9a`) — byte-identical frame chains proving typed text goes nowhere after Ctrl+N (three independent runs: a01 LEG-6 guard cascade, a03 F1/G3 pristine-base identity, a03b A0=A1=A2); the prior lab scenes had click-compensated the gap (b2g's own script clicks the composer at (800,820) after Ctrl+N).
- **Known limitations:** GPUI focus is window state — the focused tests pin the wiring (presence + ordering + bounded body), the runtime behavior verified by the Lead's N1 GUI probe at the rc.14 binary (the binding battery); `begin_projectless_chat` still has the same gap (bounded out of scope — a one-line follow-up WO candidate).
- **Provenance:** dispatched 20:50 UTC (first-attempt create — capacity freed); the first session was killed mid-flight by the Lead's own r1 stall-recovery patch (the history-stub false-death incident — the patch was reverted and redesigned the same hour); the replacement session 7937296f delivered the filled report at 22:05 through the patched filled-form completion gate; the thin bundle harvested via the workspaces API; Lead-verified diff + CI; PR #39 squash-merged at 22:39 UTC.
- **Status:** CLOSED — PR #39 (`41b9ac3`); CI green; evidence `docs/research/evidence/wo-ux-002/`.

### WO-P2-019 (CLOSED)

- **ID:** WO-P2-019 (f1-sweep renumbered follow-up; the last F1 follow-up WO of the P2 batch)
- **Title:** Confirmation-modal focus-trap completion (a11y)
- **Scope delivered:** tab/shift-tab focus traps for the four remaining confirmation modals, byte-identical clones of the evidenced six-modal family (WO-P2-011 pattern, no new abstraction): **remote pairing** (context `RemotePairingModal`, actions `RemotePairingFocusNext`/`Prev`, binding pair, panel wiring on `render_remote_pairing_modal`, handler `cycle_remote_pairing_focus`); **remote confirmation** (context `RemoteConfirmationModal`, wiring on ALL THREE variants sharing `remote_confirmation_focus` — EnableRemoteControl/DisableRemoteControl/RevokeRemoteDevice arms of `render_workspace_modal_detail`); **account logout** (context `AccountLogoutModal`, wiring on the LogOutAccount branch); **plugin install confirmation** (context `PluginInstallConfirmationModal`, wiring on its panel). 8 new actions, 8 context-scoped KeyBindings, 4 handlers, 6 wiring sites; 4 focused source-pinning tests + `key_binding_region` helper (bindings pinned to key + context by token, handler bodies shape-asserted, panel/variant wiring asserted) — `#[test]` count 135 → 139.
- **Acceptance criteria:** (1) implementing commits merged — PR #40 → `915201557c0ffe9fa4f9eff1d2b66e929360d338` (worker delivery `2574049`, single commit on base `e4a9894` exactly, ui.rs-only +435 insertions; + Lead rustfmt follow-up `31f83da` — one on_action chain pair compacted per the CI Format step, layout-only); (2) tests green — CI double matrix green at `31f83da` (Format, Clippy, full workspace Test incl. the four new trap tests, Release build, Linux startup smoke — the worker sandbox had no Rust toolchain so compile/test/fmt/clippy rode CI, the established UX-002 pattern); (3) D19 GUI evidence at `31f83da` (LEG A trap-family regression: the Clear-browsing-history modal traps Tab — Clear→Cancel→Clear wrap, Shift+Tab backward wrap, Escape close; LEG B background Tab: live bidirectional cycling on the main surface, no capture); (4) diff confined to the prescribed file; (5) §5.10 Keyboard row cells + this row + changelogs (this closure).
- **Known limitations:** the four modals are flow-gated in the lab (no backend auth/pairing/marketplace) — their runtime trap probes ride the RC binding pass (rg-a11y-02 P4 note + A11Y-BASELINE-VERDICT §5.4); screen-reader labels/AT, reduced-motion, contrast remain the bounded F1 close-out slice.
- **Provenance:** fought the platform capacity regime across 3 recycle generations (sessions 5DE83C6D → bd7534c2 → 0a136ca8; queue_watch assaults + stall_recovery r2 dead-turn recycles); the winning session 0a136ca8 (dispatched 04:12) landed at 04:24, streamed 283K chars/239 blocks, filled report at 04:38 — contract deviations NONE. Harvest via the workspaces files API (thin bundle 4876B, base prerequisite e4a9894 verified); Lead verification: full-diff source review, CI matrix (the PR-creation Actions event was swallowed once — re-fired via close/reopen, the second swallow self-resolved through the concurrency rule), guarded release build `codexrs-wo019-31f83da` (units=16/LTO=false), D19 scene with the brightness-delta button locator (full-frame VLM coordinate reads hallucinated the Clear button position twice — the sweep found it at (1080,575)).
- **Status:** CLOSED — PR #40 (`915201557c0ffe9fa4f9eff1d2b66e929360d338`); CI green both matrices; evidence `docs/research/evidence/wo-p2-019/`.
...
| 2026-09-20 | **WO-P2-020 implemented + CLOSED on merge** (PR #38 → `30572a43afcf3af6fa29d2b9da097467d74e7401`; worker r2 `a69e63b` on base `8172f6e`, r1 `8155f7c` superseded; Lead integration merge with WO-P2-018 — clean auto-merge + focused battery 5P/0F on the integrated tree, the 005/006 sequential pattern): F-A4 closed — Ctrl+P without a workspace emits the honest status “Select a workspace before searching files.” through the house SetStatus path (files_palette_command_status + state_local_workspace_cwd; workspace-open path untouched, Ctrl+K guard intact); 1 focused test + suite 226P/0F locally, CI green on the integrated head; D20 GUI evidence (verbatim status persisting at settle, no palette; Ctrl+K regression guard). §5.10 F-A4 cells flipped — the evidenced silent no-op family fully closed. Evidence `docs/research/evidence/wo-p2-020/`. |
| 2026-09-20 | **PR #34/#35 attribution re-verified (docs-only; retires the post-reset discrepancy note):** main's merge commits read `afa5b9d` = “Merge pull request #34 from payswapdotorg/feat/wo-p2-012-guard-honesty-r3” (parents `a664644` + worker r3 `aeccfc9`) and `eb348fd` = “Merge pull request #35 from payswapdotorg/docs/f1-sweep-inventory” (parents `afa5b9d` + `00e684d`) — consistent with refs/pull/34/head = `aeccfc9` and refs/pull/35/head = `00e684d`; the existing ledger/report citations “WO-P2-012 … PR #34 → `afa5b9d`” are CORRECT; the earlier off-by-one suspicion was a misreading. No row edits required. |
| 2026-09-20 | **WO-UX-002 implemented + CLOSED on merge** (PR #39 → `41b9ac3`; worker `18a1815` on base `c8f6831`, ui.rs-only +102, CI green full matrix): keyboard-only chat creation — the RG-A11Y N1 gap closed by the with_prompt focus pattern after the BeginNewChat dispatch; 2 focused source-pin tests + reducer asserts. The finding's own battery (RG-A11Y-01/02/03/03b, 69 frames + targeted VLM reads at `8a68c9a`) recorded as the evidence baseline with findings N1–N4 and three new lab laws (route persistence in the shared sqlite; the deterministic renderer making cross-boot md5 equality a sound no-op proof; seeded sessions are not live tasks). Provenance: the first session was false-killed by the Lead's r1 stall-recovery patch (reverted + redesigned to the batch-length signal the same hour); the replacement delivered through the patched filled-form gate. Evidence `docs/research/evidence/wo-ux-002/`. |
| 2026-09-21 | **WO-P2-019 implemented + CLOSED on merge** (PR #40 → `9152015`; worker `2574049` on base `e4a9894` exactly, ui.rs-only +435 insertions + Lead rustfmt `31f83da`): tab/shift-tab focus traps for the four remaining confirmation modals — remote pairing / remote confirmation (all three variants sharing the focus handle) / account logout / plugin install confirmation — byte-identical clones of the evidenced WO-P2-011 six-modal family (8 actions, 8 context-scoped bindings, 4 handlers, 6 wiring sites; 4 focused source-pinning tests + key_binding_region helper, 135→139); worker sandbox had no Rust toolchain — compile/test/fmt/clippy rode the CI double matrix (green at `31f83da`); D19 GUI evidence at the guarded-built binary (LEG A: the destructive-modal family still traps — Clear→Cancel→Clear wrap + backward wrap + Escape; LEG B: background Tab live and bidirectional, no capture); the four flow-gated modals' runtime probes scheduled at the RC binding pass. Harvest through 3 capacity-regime recycle generations (winning session 0a136ca8, 283K chars, contract deviations NONE). Evidence `docs/research/evidence/wo-p2-019/`. |
| 2026-09-21 | **WO-P2-017-FIX implemented + CLOSED on merge — the LAST P2 follow-up** (PR #41 → `8dcfcb9`; worker `a079caf` on base `c2de31e` exactly — re-verified at clone time per the WO's own rule, ui.rs-only +298/−25 + Lead rustfmt `1886ce2`): palette and keyboard-shortcuts-overlay close restore focus to the previously focused surface (else the composer house default, else no-element-focus), and the r1 deterministic palette-close panic (gpui entity-map double-lease on the workspace-side `palette.read(cx)`) eliminated by `CommandPaletteCloseContext` parameter-passing — capture-before-update in both palette-side close paths, idle-read helper for the workspace-side paths, compile-time fn-pointer signature pin + capture-mapping regression + the two r1 focus-contract tests ported (CI double matrix green at `1886ce2`, the no-toolchain worker per the UX-002/019 pattern); D17 GUI evidence at the guarded-built `codexrs-wo017fix-1886ce2`: Ctrl+K → Escape closes with NO PANIC (the r1 reproduction), typed probes "x"/"y" land in the composer (both VLM-adjudicated), clears return the exact closed-state md5. r1 (`5d3ab2e`) had been REJECTED at Lead verification (unit tests green, binary crashed); the verified-design addendum rode every r2 dispatch. Harvest through ~13 capacity-regime recycle generations (winning session 2d94b1c4, 179K chars, contract deviations NONE; an earlier server-confirmed COMPLETE forensically closed as report-shaped failure — no branch push, the harvest-promptly lesson institutionalized). Evidence `docs/research/evidence/wo-p2-017/`. |
