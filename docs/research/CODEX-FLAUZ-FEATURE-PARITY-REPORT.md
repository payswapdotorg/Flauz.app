...
| Capability | Official Codex | Flauz | Platform | Backend | UI | Discoverability | UX parity | Functional parity | Recovery | Evidence | Gap |
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| In-app browser | Native browser panel with bounded tabs, URL/navigation state, JPEG frame streaming, keyboard/pointer/scroll input vs supervised isolated Edge/Chrome/Chromium profile; `Open browser tab` Ctrl/Cmd+T, `Toggle browser panel` Ctrl/Cmd+Shift+B (baseline-verified bindings) `[historical-record] A §4`; **26.727: address bar revisits browsing history or searches Google on no match; browsing-history management in Settings; agent can search history** `[docs-derived] A §4]`**Linux preview runtime (WO-LAB-001): shortcuts overlay lists "Reload Browser Page" Ctrl+R and "Force Reload Browser Page" Ctrl+Shift+R** `[runtime-observed: linux-preview 26.908.70816; evidence docs/research/evidence/codex-linux/`06]` | Native GPUI panel: `BrowserSession::spawn`/navigate/back/forward/reload/stop, tabs, downloads store (`browser_downloads` table) `[source-derived] browser.rs, B2 §4; runtime-observed DB schema]`; panel is `InspectorPane::Browser` core lib.rs:259-267 `[source-derived]`; **Ctrl+T / Ctrl+Shift+B on the entry surface are silent no-ops; `OpenBrowserTab` refused without an open chat — "Open a chat before opening the Browser." core lib.rs:14521** *(pre-wave evidence; FIXED by WO-P1-002 — merged d15333e 2026-09-17 — toggles now surface the guard visibly, ev wo-p1/12)* `[runtime-observed] b2g-06; source-derived]`; browsing-history store + Settings management on main (WO-P2-009, merged `5302e7e` + `fe3903e`): schema v5 `browsing_history` ring (MAX_BROWSING_HISTORY), address-bar revisit matching, Settings rows + Clear modal `[source-derived ui.rs/core; runtime-observed: evidence wo-p2-009/ — d12f rows render titles/URLs (VLM-read), Clear modal verbatim, cleared empty-state + disabled Clear]`; browser-pane chords (WO-P2-011, merged `3c812a7`): context-scoped interceptor arms — Ctrl+R reload, Ctrl+Shift+R force-reload, Ctrl+Shift+C copy-URL — resolved only while the browser pane holds focus (the "Browser" key context), ahead of the registry lookup; outside that focus the chords fall through untouched (Ctrl+Shift+C keeps its registry meaning copyWorkingDirectory — the official dual-meaning chord); honest no-page guards ("Open a page before reloading the Browser." / "Open a page before copying the Browser address.") `[source-derived ui.rs/core lib.rs:5686/15226; unit-covered on CI (3 tests); D14/D14b fall-through + no-hijack byte-identical on the real binary, guarded-row honest absence consistent with D13: evidence wo-p2-011/]` | win `[historical-record]`; linux `[runtime-observed no-op]` | partial | partial | complete (WO-P1-002 CLOSED — merged d15333e 2026-09-17): persistent sidebar affordance renders in the default Tasks layout (ev wo-p1/01, VLM-read "Browser" row above Settings); entry-surface toggles surface the guard visibly (ev wo-p1/12) — GUI evidence docs/research/evidence/wo-p1/ | partial | partial | — | PM "In-app browser"; A §4; B2 §4 + Finding 2; C2 anchor re-verification | ui-integration — P1 → **WO-P1-002 CLOSED (merged d15333e718a7649238f33a183af27c885dbaa8aa via PR #13; unit+runtime evidence; CI green windows-latest + ubuntu-24.04; GUI evidence docs/research/evidence/wo-p1/)**; ux — P2 address-bar history + Settings management **CLOSED (WO-P2-009 merged via PR #24 → `5302e7e` + integration-fix PR #27 → `fe3903e`, 2026-09-19; CI green both matrices; evidence wo-p2-009/)**; residual: Google-search fallback on no-match revisit (unverified reference behavior), browser-panel surfaces runtime-gated (lab-documented); reload/force-reload/copy-URL chords **CLOSED (WO-P2-011 merged via PR #31 → `3c812a7`, 2026-09-19; CI green both matrices; evidence wo-p2-011/)**; residual: no-cache force-reload variant needs platform-side CDP `Page.reload {ignoreCache: true}` (documented at the seam — delayed capability, never faked) |
...
| Keyboard and accessibility | Ctrl/Cmd+K, Shift+P, G, P search-files drill-in, arrows/Enter/Escape; dynamic Settings group in palette; editable shortcut registry with stable grouping; complete focus order, screen-reader labels `[historical-record] A §10]; current docs add Clear terminal (Ctrl+L/Ctrl+K), font-size Ctrl+±/0, Toggle file tree Ctrl+Shift+E (post-baseline, intro dates `[unverified]`) `[docs-derived]`; **Linux preview runtime (WO-LAB-001, 2026-09-17): Ctrl+/ opens a searchable "Keyboard shortcuts" overlay ("Search shortcuts") with 22 runtime-read rows — Chat (New chat Ctrl+N/Ctrl+Shift+O; Archive chat Ctrl+Shift+A; New standalone chat Ctrl+Alt+O; Toggle pin Ctrl+Alt+P), Navigation (Find Ctrl+F; Back Ctrl+[/Mouse Back; Forward Ctrl+]/Mouse Forward; Next/Previous recently viewed chat Ctrl+Tab/Ctrl+Shift+Tab; Switch to Work Alt+2), General (Close Tab Ctrl+W/Ctrl+F4; Copy deeplink Ctrl+Alt+L; Copy working directory Ctrl+Shift+C; Reload/Force Reload Browser Page Ctrl+R/Ctrl+Shift+R; Open command menu Ctrl+K/Ctrl+Shift+P; Rename chat Ctrl+Alt+R; Search Files... Ctrl+P; Show keyboard shortcuts Ctrl+/; Toggle File Tree Ctrl+Shift+E — runtime-confirming the docs-derived file-tree binding; Clear terminal/font-size rows not in the captured sections)** `[runtime-observed: linux-preview 26.908.70816; evidence docs/research/evidence/codex-linux/`05-07]` | Native command palette with verified stable registry subset (70-command registry — 51 base + WO-P2-010’s 19 evidenced registry rows; RWO-021 fresh-verification pass verified `PaletteCommand::ALL` = 70 at ui.rs:3480; **every default-nav settings section indexed — on main (WO-P2-004, merged `7aa7163` via PR #16, 2026-09-17): six new entries (Profile, Import, Browser, Configuration, Hooks, Git) in the Settings palette group with title/description/icon mirroring the settings-nav rows, no guards (unauthenticated + repository-independent, mirroring the official login-surface reference); Personalization palette title aligned with its settings-nav label; `SettingsSection::DEFAULT_NAV_SECTIONS` registry with coverage + filtering tests** `[source-derived ui.rs:3262+; runtime-observed: evidence wo-p2-004/ — palette Settings group lists all 11 entries (folded + scrolled views); queries “import”/“profile”/“browser”/“configuration”/“hooks”/“git” resolve and navigate to their pages unauthenticated — the ev/18 gap remediated]`); Ctrl+/ overlay + editable settings page `[runtime-observed ev/11, ev/15]`; registry-command palette rows on main (WO-P2-010, merged `876bbe8`): review/fork/copy×3/approval×2/rename/goto-chat-N rows with per-command availability guards `[source-derived ui.rs; runtime-observed: evidence wo-p2-010/ — D13 guards live at the entry surface, "Go to chat 1" verbatim]`; pending remaining unverified reference commands, complete focus order, screen-reader labels, reduced-motion | win `[historical-record]`; linux `[runtime-observed]` | partial | partial | partial (settings-page palette indexing CLOSED by WO-P2-004; silent-state no-op residuals F-A1/A2/A3/A6/F-D1/F-D2 **CLOSED (WO-P2-012 merged via PR #34 → `afa5b9d`, 2026-09-19: per-guard honest statuses via Action::SetStatus — "Select a chat before archiving/pinning or unpinning/renaming it.", "The selected chat is no longer available.", "A Git workflow is already running."; typed /review falls through to a visible message submission (ReviewSlashCommandAction enum) + honest review-start statuses; /compact while the thread runtime loads reports composer_error guidance and recovers on load — 8 focused tests; D15 evidence wo-p2-012/)**; F-A4 (Ctrl+P no-workspace) **CLOSED (WO-P2-020 r2 merged via PR #38 → `30572a43afcf3af6fa29d2b9da097467d74e7401`, 2026-09-20: the Files palette early-return now surfaces the honest status “Select a workspace before searching files.” through Action::SetStatus — the house status path (WO-P2-008/012 copy family); the workspace-open path untouched; D20 evidence wo-p2-020/)**; F-A5 (empty slot Ctrl+1..9) documented as plausibly-by-design; + 23 bound-but-never-handled GPUI actions + unbound OpenBrowserTabShortcut — full enumeration at evidence wo-p2-007/input-surface-sweep.md, recorded per RWO-021 KSR-C5; no-toolbar-button residual **CLOSED (WO-P2-018 merged via PR #37 → `b562397ce6325f12d21f67f0549f5860b7801306`, 2026-09-20: TitleBarDiscoveryEntry chrome buttons — the visible palette entry + the Activity bell dispatching the existing actions; archived-deletion visible “Delete” label + unread-dot tooltip “Unread activity”; D18 evidence wo-p2-018/)**; confirmation-modal focus traps **CLOSED (WO-P2-019 merged via PR #40 → `915201557c0ffe9fa4f9eff1d2b66e929360d338`, 2026-09-21: tab/shift-tab traps for the four remaining confirmation modals — remote pairing, remote confirmation (all three variants), account logout, plugin install — context-scoped clones of the evidenced family; 4 source-pinned tests; CI double matrix; D19 evidence wo-p2-019/ — the destructive-modal family still traps at the merged tree and background Tab stays live; the four flow-gated modals' runtime probes at the RC binding pass)**; palette/overlay close focus restoration **CLOSED (WO-P2-017-FIX merged via PR #41 → `8dcfcb97b34bc9d4fd73bee534628d5e9c789abe`, 2026-09-21: the command palette and keyboard-shortcuts overlay close paths restore focus to the previously focused surface — else the composer house default when rendered, else no-element-focus so window chords keep working; the r1 deterministic palette-close panic (entity-map double-lease on the workspace-side `palette.read(cx)`) eliminated by `CommandPaletteCloseContext` parameter-passing with a compile-time fn-pointer signature pin; 4 focused tests; CI double matrix; D17 evidence wo-p2-017/ — Ctrl+K→Escape no panic, typed probes land in the composer, VLM-adjudicated)**; focus order / screen-reader labels / reduced-motion pending) | partial | partial | — | PM "Keyboard and accessibility" + §"Keyboard accessibility baseline"; A §10; B2 §10 ev/08-11/15/18 | ux — P2 palette settings-page indexing **CLOSED (WO-P2-004 merged via PR #16 → `7aa7163`, 2026-09-17): every default-nav settings section indexed (six new entries; Personalization title aligned with its nav label; DEFAULT_NAV_SECTIONS registry with coverage + filtering tests); CI green both matrices; runtime evidence docs/research/evidence/wo-p2-004/**; input-quality fix **CLOSED (WO-P2-007 merged via PR #20 → `a3c0e01`, 2026-09-18)**: the direct Ctrl+P keypress now routes through the `searchFiles` interceptor arm — dead `OpenFileSearch` action removed (mirroring Ctrl+K/G), 5-assertion regression test, D10b lab evidence (workspace-seeded Ctrl+P opens the "Search files" palette, VLM-read; no-workspace residual documented as F-A4); P2 palette rows for the evidenced registry commands **CLOSED (WO-P2-010 merged via PR #25 → `876bbe8`, 2026-09-19; availability guards per command; D13 evidence)**; silent no-op family **CLOSED (WO-P2-012 via PR #34 → `afa5b9d`, 2026-09-19; F-A4 **CLOSED (WO-P2-020 r2 via PR #38 → `30572a43afcf3af6fa29d2b9da097467d74e7401`, 2026-09-20 — honest no-workspace status, D20 evidence)**, F-A5 documented-by-design)**; visible-entry + label parity **CLOSED (WO-P2-018 via PR #37 → `b562397ce6325f12d21f67f0549f5860b7801306`, 2026-09-20: title-bar palette entry + Activity bell + archived-deletion label + unread-dot tooltip; D18 evidence wo-p2-018/)**; confirmation-modal focus traps **CLOSED (WO-P2-019 via PR #40 → `9152015`, 2026-09-21: the four remaining modals trapped, context-scoped; D19 evidence wo-p2-019/)**; palette/overlay close focus restoration **CLOSED (WO-P2-017-FIX via PR #41 → `8dcfcb9`, 2026-09-21: r1 rejected for the close-path panic; r2 re-ports the contract and eliminates the panic via parameter-passing; D17 evidence wo-p2-017/)**; P2 residuals (focus order, screen-reader labels, remaining unverified reference commands); P3 (Clear terminal / font-size / file-tree shortcut deltas) |
...
| Activity view & unread attention **(added by C2 audit)** | **26.727: Activity view** — bell icon / Ctrl/Cmd+Alt+U shows recently engaged chats needing attention; Shift+Esc clears unread indicators; Ctrl/Cmd+Alt+A next chat needing attention `[historical-record + docs-derived] A §10]` | Unread-attention session state + four registry bindings **on main (WO-P2-008, merged via PR #23 → `00a3392`, 2026-09-19)**: bounded `needs_attention_task_ids` (capped `MAX_VISIBLE_THREADS`, not persisted) flagged on background turn completion (failed turns included — attention regardless of outcome) + approval requests in non-selected chats; visit clears, archive drops; sidebar 6px dot + medium-weight title; `toggleThreadUnread` Ctrl+Shift+U (round-trip + honest no-selection status), `nextUnreadChat` Ctrl+Alt+A (cyclic sidebar-order jump, selected chat never a candidate, honest empty statuses), `clearAllUnread` Shift+Escape (honest count incl. "No unread chats"), `toggleActivityView` Ctrl+Alt+U — **real Activity view surface on main (WO-P2-013, merged via PR #36 → `c10b426`, 2026-09-20)**: centered bounded panel listing the attention-flagged chats in sidebar order (pinned first), per-row status icon + medium-weight title + attention dot + relative time, count label, keyboard footer; full keyboard path (scoped up/down/enter/escape, wrap selection, bare-Escape close precedence — Shift+Esc preserved for clear-all), Enter/click jumps via the exact `nextUnreadChat` path (visit-clears resolves the flag, the surface dismisses), honest empty state ('No chats need attention' + mark-unread guidance), command palette row 'Toggle Activity view' (Bell icon, registry copy verbatim); no second event store, no semantic changes (non-persisted `activity_view_visible` flag only); `MAX_KEYBOARD_SHORTCUT_COMMANDS` 71→76; 7 core state tests + 5 app binding tests (RWO-022 FW-1 correction: five delivered, not six) + 6 Activity-surface tests (WO-P2-013) `[source-derived ui.rs/lib.rs; runtime-observed: evidence wo-p2-008/ — D11b all four bindings resolve visibly (VLM-read verbatim statuses), D11 honest no-selection path, D11r full flow: dot/jump/clear on real thread rows with the restored pinned runtime]` | win, macos `[docs-derived]`; linux `[source-derived + runtime-observed]` | partial | partial | partial (attention state + bindings + Activity view surface delivered WO-P2-008/WO-P2-013) | partial | partial | — | A §10 Sidebar; B2 shortcut inventory; C2 source sweep | ux — P2 unread-attention state + bindings **CLOSED (WO-P2-008 merged via PR #23 → `00a3392`, 2026-09-19; CI green both matrices; evidence wo-p2-008/)**; Activity view surface **CLOSED (WO-P2-013 merged via PR #36 → `c10b426`, 2026-09-20; CI green both matrices; evidence wo-p2-013/ — D16: open/nav/jump/resolve/count/empty-state all VLM-verified on the delivery binary)**; residuals: unread-state persistence across restarts (reference behavior unverified), bell chrome icon + sidebar-dot-click contextual discovery (WO-P2-020 follow-up material); dot-on-row + jump-between-chats GUI observation **CLOSED by the D11r runtime-restored re-run** (RWO-022 FW-5/FW-7, 2026-09-19: pinned Codex CLI @0.146.0-alpha.3.1 restored to the lab, `CODEX_RS_CODEX_BIN` + pre-created `CODEX_HOME`, ctrl+Return submits — dot-on-row, dot-persists, jump-with-visit-clear, re-mark, clear-all-with-honest-count all VLM-read on real thread rows; evidence wo-p2-008/d11r/) |
...
with Flauz-side absence source-verified at `f113515` before declaring.
  (Historical census at A/B-audit time; at the current head side chats is
  `complete` — WO-P2-006, PR #18 → `5287c29f3f` — and Activity view is
  `partial` — WO-P2-008, PR #23 → `00a3392` — leaving 4 `missing` rows;
  the counts table above is authoritative. [RWO-022 FW-4])
...
3. Multi-folder local projects — in-baseline official capability (26.715)
   wholly absent (single-path project model) → **CLOSED (WO-P1-003 merged
   via PR #15 → `d06ae3b`, 2026-09-17)**: model + Edit project surface +
   primary-swap re-key + related-folder file search + storage schema v4 on
...
4. Command palette does not index all settings pages ("import" → No matches
   ...
   (WO-P2-004 merged via PR #16 → `7aa7163`, 2026-09-17)**: every
   default-nav settings section indexed in the palette (six new entries:
   Profile, Import, Browser, Configuration, Hooks, Git; Personalization
   title aligned with its nav label; DEFAULT_NAV_SECTIONS registry with
   coverage + filtering tests); CI green both matrices; runtime evidence
   docs/research/evidence/wo-p2-004/ (15 VLM-read captures; the ev/18
   "import" gap remediated end-to-end).
5. Slash-command coverage delta vs current 24-command set (11 literal names
   absent; subset implementable now) → **CLOSED (WO-P2-005 merged via PR
   #17 → `e46ad4f3df`, 2026-09-18)**: the four implementable names shipped
   guarded (`/approve /fast /personality /worktree`) + the fork-destination
   picker GUI fix (4726dd4); seven names stay open (deferred capabilities /
   unverified semantics — enumerated on the §5.2 Composer row) [post-WO-P2-006: six — `/side` shipped; RWO-022 FW-2]; CI green
...
12. Browser address-bar history / Google fallback (26.727) — history + Settings management CLOSED (WO-P2-009: PR #24 → `5302e7e` + integration-fix PR #27 → `fe3903e`); residual: Google no-match fallback (reference behavior unverified). [RWO-022 FW-3]
...
| J4 Git | Snapshot → worktree fork → diffs → review → commit/push → PR `[historical-record]` | Repository page (stats, changed files, branches, worktree form, bounded diff review, "Commit or push") renders (ev/02); PR page renders then honest gh-missing banner (ev/03) | Works (commit/push prior-wave validated `[historical-record]`) | gh CLI absent in lab (honest, actionable banner) | None; multi-repo review unblocked (WO-P1-003 CLOSED, d06ae3b) |
...
17. **Runtime finding during the WO-P1-003 lab scene (2026-09-17): Ctrl+P
    ...
    **FIXED (WO-P2-007, PR #20 → `a3c0e01`, 2026-09-18):** the dead
    `OpenFileSearch` action + binding are removed; the interceptor
    registry's `searchFiles` command (already carrying CmdOrCtrl+P) owns
    the key — mirroring Ctrl+K/Ctrl+G. Regression test
    `ctrl_p_routes_to_the_search_files_command`; LINUX_GUI_LAB D10b
    (workspace-seeded): Ctrl+P opens the "Search files" palette
    (VLM-read), Escape closes byte-identical. Residual: with NO workspace
...
- 2026-09-19 (WO-P2-012 merge closure): the six evidenced silent no-op
  states CLOSED on main — merge commit afa5b9d (PR #34; r3 reconciliation
  aeccfc9 over base a664644, superseding the r1/r2 unmerged branches); CI
  green on windows-latest and ubuntu-24.04; §5.10 Keyboard row Flauz cell +
  Gap cell updated (F-A4 → WO-P2-020 decision, F-A5 documented); FW-9
  executor fall-through tests delivered in-scope (RWO-022 follow-up
  complete); D15 guard-honesty lab evidence at
  docs/research/evidence/wo-p2-012/ (F-D1 fall-through VERIFIED on the real binary; guard-family SetStatus mechanism VERIFIED at the entry surface on the r3 binary (d15d d07); F-A1/A2/A3 chord paths shadow-blocked by the pre-existing bound-but-never-handled KeyBinding class (A/B-identical on 3c812a7); F-D2 NOT OBSERVED in lab; F-A6 NOT RUN offline).
...
| 2026-09-17 | **Merge closure:** WO-P1-003 CLOSED (PR #15 → `d06ae3b`; CI green both matrices at d280a4b); §5.1 row + §5.2 + §8.2/J1/J4 flipped to CLOSED; five closure gates satisfied; multi-repo review (26.727) unblocked as its own future order. |
| 2026-09-17 | **Merge closure:** WO-P2-004 CLOSED (PR #16 → `7aa7163`; CI green both matrices at 6bb619d); §5.10 row + §5.9 cross-ref + §8.2 item 4 flipped to CLOSED; five closure gates satisfied; P2 progress 1/3. |
| 2026-09-17 | **WO-P2-004 implemented (PR #16: 5d4b083):** palette settings-page indexing — every default-nav settings section reachable from the command palette (six new entries Profile/Import/Browser/Configuration/Hooks/Git; `PaletteCommand::ALL` 45 → 51; Personalization title aligned with its nav label; `DEFAULT_NAV_SECTIONS` registry + coverage/filtering tests); §5.10/§5.9/§8.2 updated; runtime evidence wo-p2-004/ (15 VLM-read captures; the ev/18 "import" → No-matches gap remediated; queries navigate unauthenticated, mirroring the official login-surface reference). Row flip to closed on merge. |
...
| 2026-09-18 | **WO-P2-005 implemented (PR #17: 2e6358f + GUI fix 4726dd4)**: slash-command coverage — `/approve /fast /personality /worktree` (guarded) + menu rows + unit tests; LINUX_GUI_LAB D8 found the fork-picker wiring defect (picker closed for `/worktree`); fix 4726dd4 = testable `composer_keeps_fork_picker` predicate + regression test, 15-site lifecycle audit clean; §5.2 Composer row + §8.2 item 5 updates; [RWO-022 FW-4 §-ref fix] runtime evidence wo-p2-005/ (D8c4). Row flip to closed on merge. |
| 2026-09-18 | **Merge closure:** WO-P2-005 CLOSED (PR #17 → `e46ad4f3df`; CI green both matrices); §5.2 Composer row + §8.2 item 5 flipped to CLOSED; [RWO-022 FW-4 §-ref fix] five closure gates satisfied; P2 progress 2/3. |
| 2026-09-18 | **WO-P2-006 implemented (PR #18: 764e4db + Lead rustfmt 73eb55c)**: side chats — Ctrl+Alt+S binding (bind_keys + interceptor registry, Thread group) + guarded `/side` (21 named commands); side submit creates the side thread without selecting it; main-chat selection/active-turn untouched (state test); close returns to main view; §5.1 Side chats row + §8.2 item 6 updates; [RWO-022 FW-4 §-ref fix] runtime evidence wo-p2-006/ (D9/D9b VLM-read). Row flip to closed on merge. |
| 2026-09-18 | **Merge closure:** WO-P2-006 CLOSED (PR #18 → `5287c29f3f`; CI green both matrices); §5.1 Side chats row flipped missing → complete; [RWO-022 FW-4 §-ref fix] §8.1 counts (complete 7→8, missing 6→5); §8.2 item 6 flipped to CLOSED; five closure gates satisfied; P2 with-work-orders 3/3 complete. |
...
| 2026-09-18 | **Merge closure:** WO-P2-007 CLOSED (PR #20 → `a3c0e01`; CI green both matrices); §5.10 Keyboard row + §9 override 17 updated to FIXED; [RWO-022 FW-4 §-ref fix] Wave R research landed (PR #21 sweep evidence `d659eb3`, PR #22 reference research `bf61964`). |
...
| 2026-09-19 | **WO-P2-008 implemented + CLOSED on merge** (PR #23 → `00a3392`; Worker delivery c37c21b + Lead verification; CI green both matrices): unread-attention session state (bounded, visit-clears, archive-drops; background turn completion incl. failed turns + approval requests flag non-selected chats) + four registry bindings (Ctrl+Shift+U mark-unread round-trip, Ctrl+Alt+A next-unread cyclic jump, Shift+Escape clear-all honest count, Ctrl+Alt+U Activity-view honest guidance) + sidebar dot/medium-weight title; MAX_KEYBOARD_SHORTCUT_COMMANDS 71→76 (registry 76). Gates: fmt ✓, 7/7 core state tests (per-test verified), 5 app binding tests (CI; RWO-022 FW-1 correction), guarded build ✓, D11/D11b lab scenes (VLM-read verbatim statuses; D11 documents the no-runtime-lab limitation honestly — dot-on-row unit-covered). §5.10 row missing → partial; §8 counts missing 5→4, partial 31→32; §9 override 18; §8.2 P2 item 8 rescoped to the Activity view surface. Duplicate delivery 64b28f6 (the orphaned re-dispatch's delayed sandbox push, 2026-09-19 00:14) adjudicated redundant — the verified lineage merged. |
| 2026-09-19 | **WO-P2-009 implemented + CLOSED on merge** (PR #24 → `5302e7e` + integration-fix PR #27 → `fe3903e`; CI green both matrices): browsing-history store (schema v5 ring), address-bar revisit matching, Settings management + Clear modal; the D12 five-cycle layout war (byte-identical dead theories → pixel forensics → palette-convention fix) recorded in evidence wo-p2-009/; §5.4 browser row ux note updated (history/Settings CLOSED; Google-fallback + browser-panel residuals runtime-gated). |
| 2026-09-19 | **WO-P2-010 implemented + CLOSED on merge** (PR #25 → `876bbe8`; CI green both matrices): palette rows for the evidenced registry commands with per-command availability guards (D13: honest absences at the entry surface; "Go to chat 1" verbatim; chat-scoped present-rendering runtime-gated); §5.10 keyboard row note updated. |
| 2026-09-19 | **WO-REVIEW-001 deep phase armed:** RWO-020 reference-behavior review rubric merged (PR #26 → `1b0500e`; rubric.md + rubric-r2.md on main); RWO-021-B fresh-verification pass dispatched (in flight); RWO-022-C adversarial pass staged on the post-#27 head. |
| 2026-09-19 | **RWO-021 fresh-verification pass delivered** (PR #28 → `61c491c`): no product-code defect in the 8 complete rows; feasible battery green (core 231 / protocol 60 / storage 19); MAX 71→76 quirk verified cured; 3 documentation defects (2 remediated in this commit, 1 pre-remediated `dbee361`); advisory residuals queued for wave-T re-scope. Evidence rwo-020/verification.md. |
| 2026-09-19 | **RWO-022 adversarial pass delivered** (PR #29 → `230f6fc`): all five P2 closures HELD on substance; zero product-code defects; EQ-1 resolved (lab environment drift D9-online vs D11-offline); 9 follow-ups (FW-1..FW-4 + FW-6/FW-8 remediated in this commit; FW-5/FW-7 lab items; FW-9 folds into WO-P2-012). Evidence rwo-020/adversarial.md. |
| 2026-09-19 | **RWO-021 verification-r2 delivered** (PR #30 → `bb52d4d`): sysroot build exit 0; full battery 639/0/2 across five crates; ten fresh GUI scenes; invariants 76=76=76, zero dead commands, PaletteCommand::ALL=70. Evidence rwo-021/. |
| 2026-09-19 | **WO-REVIEW-001 CLOSED:** all legs merged (sweep `3c9f113`-base; rubric `1b0500e`; verification `61c491c` + `bb52d4d`; adversarial `230f6fc`). Zero product-code defects; all documentation defects remediated; open items: FW-5/FW-7 lab, FW-9 → WO-P2-012, Activity-view surface future WO. |
| 2026-09-19 | **WO-P2-011 merged** (PR #31 → `3c812a7`): §5.4 browser row — browser-pane chords clause added (context-scoped interceptor arms, dual-meaning preservation, honest guards) + resolution cell: chords CLOSED, no-cache residual documented; §5.4 009-era scope phrase corrected in the work-order ledger with attribution. |
| 2026-09-19 | **RWO-022 FW-5 + FW-7 closed (lab follow-ups):** D11 script key defect fixed in-place (ctrl+Return submits; d11/ frames relabeled honestly — 02b/04 creation claims corrected, manifest completed with 02a); lab Codex CLI runtime restored + pinned (`@openai/codex@0.146.0-alpha.3.1-linux-x64`, sha256 `ae77c5e7…`, `CODEX_RS_CODEX_BIN` + pre-created `CODEX_HOME` — negative control proves the home requirement) and the D11 full-flow scene re-run as D11r at main `3c812a7`: footer "App-server online" (EQ-1 drift reversed), dot-on-row / dot-persists / jump-with-visit-clear / re-mark / clear-all-honest-count all runtime-observed on real thread rows. §5.10 residual cell closed. Evidence wo-p2-008/d11r/. |
| 2026-09-20 | **WO-P2-013 merged (Activity view surface, J-17):** PR #36 → `c10b426` (worker final `20a6018` on base `a664644` + Lead rustfmt `6d08bf3`; five worker iterations through the platform's stream-death cycle — superseded attempts self-archived; final session pushed 04:27:45 after its 04:00 husk, stream-death ≠ work-death #3; redundant queue machinery stood down by the Lead). Real bounded Activity surface on the existing attention primitives: toggle + idempotent close, sidebar-order rows from `needs_attention_task_ids` (no second store), jump via the exact `nextUnreadChat` path with visit-clear resolution, full keyboard path with Shift+Esc preserved, honest empty state, palette row (registry copy verbatim). CI double-green; D16 evidence all-VLM-verified. Projects-and-chats row D-2 staleness fix applied with this closure (manual project ordering GUI-002 + unread state WO-P2-008 remainders removed from the row tail). |
| 2026-09-20 | **WO-P2-018 implemented + CLOSED on merge** (PR #37 → `b562397ce6325f12d21f67f0549f5860b7801306`; worker delivery `0ece5bb`, single commit on base `8172f6e` exactly, ui.rs-only +241/−15): visible-entry + label parity — TitleBarDiscoveryEntry model + `render_command_palette_entry_button` (native icon Button, navigate-back/forward tooltip style) + `render_activity_view_bell_button` (the J-17 surface) + `UNREAD_ATTENTION_DOT_TOOLTIP` “Unread activity” + `ARCHIVED_CHAT_DELETE_LABEL` visible Delete/Unarchive labels; 4 focused tests + full suite 229P/0F locally, CI double matrix green; D18 GUI evidence (title-bar entries render; Search opens the Unified palette; Bell opens the Activity view honest empty state; dot tooltip verbatim; archived section visible labels + “Delete all” header; geo-probe cross-check). Evidence `docs/research/evidence/wo-p2-018/`. |
| 2026-09-20 | **Merge closure:** WO-P2-018 CLOSED (PR #37 → `b562397ce6325f12d21f67f0549f5860b7801306`; CI green both matrices); §5.10 Keyboard row no-toolbar-button residual + resolution cell flipped to CLOSED. |
| 2026-09-20 | **WO-P2-020 implemented + CLOSED on merge** (PR #38 → `30572a43afcf3af6fa29d2b9da097467d74e7401`; worker r2 `a69e63b` on base `8172f6e`, r1 `8155f7c` superseded; Lead integration merge with WO-P2-018 — clean auto-merge, focused battery 5P/0F on the integrated tree, the 005/006 sequential pattern): F-A4 closed — Ctrl+P without a workspace surfaces the honest status “Select a workspace before searching files.” via `files_palette_command_status` → Action::SetStatus (house status path); `state_local_workspace_cwd` helper; workspace-open path untouched (Ctrl+K guard intact); 1 focused test + suite 226P/0F locally, CI green on the integrated head; D20 GUI evidence (verbatim status persisting at settle, no palette, Ctrl+K regression guard). Evidence `docs/research/evidence/wo-p2-020/`. |
| 2026-09-20 | **Merge closure:** WO-P2-020 CLOSED (PR #38 → `30572a43afcf3af6fa29d2b9da097467d74e7401`); §5.10 F-A4 gap + resolution cells flipped to CLOSED — the evidenced silent no-op family (F-A1/A2/A3/A6/F-D1/D2/F-A4) is now fully closed. |
